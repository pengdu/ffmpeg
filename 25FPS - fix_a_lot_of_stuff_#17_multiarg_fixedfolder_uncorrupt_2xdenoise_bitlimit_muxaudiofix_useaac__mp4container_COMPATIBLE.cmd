...
::-------------------------------------------------------------------------------flags
  set ARGS=%ARGS% -flags            "+low_delay+global_header+ilme+low_delay-unaligned-cgop+loop-output_corrupt"
  set ARGS=%ARGS% -flags2           "+fast+ignorecrop+showall+export_mvs"
  set ARGS=%ARGS% -fflags           "+autobsf+genpts+discardcorrupt+fastseek-igndts-ignidx-nofillin"
...
::fix audio bitstream when possible
  if /i ["%PROBED_VCODEC%"] EQU ["aac"] (
    set ARGS=%ARGS% -bsf:a                  "aac_adtstoasc"
  ) else if /i ["%PROBED_VCODEC%"] EQU ["mp3"] (
...
::set ARGS=%ARGS% -analyzeduration  "60000000"
...
::-------------------------------------------------------------------------------limiting bitrate (video)
  ...
  set ARGS=%ARGS% -maxrate:v        "800k"
  set ARGS=%ARGS% -bufsize:v        "1500k"
...
::-------------------------------------------------------------------------------encoders
  set ARGS=%ARGS% -codec:v          "libx264"



::-------------------------------------------------------------------------------audio processing

  set ARGS=%ARGS% -codec:a        "aac"
...
::call ffmpeg.exe -y -hide_banner -loglevel "info" -stats -threads "16" -strict "experimental" -i "%FILE_INPUT%" %ARGS% "%FILE_OUTPUT%"
  call ffmpeg.exe -y -hide_banner -loglevel "info" -stats -threads "16" -strict "very"         -i "%FILE_INPUT%" %ARGS% "%FILE_OUTPUT%"
@echo off
echo.
title Done.
