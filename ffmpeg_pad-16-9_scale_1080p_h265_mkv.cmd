...
call ffmpeg -hide_banner -y -flags "-output_corrupt" -fflags "+discardcorrupt" -i "%INPUT%" -tag:v hvc1 -c:v libx265 -c:a copy -crf 26 -preset veryslow -vf "fifo,setpts=PTS-STARTPTS,format=yuv420p,pad=width=ih*16/9:height=ih:x=(ow-iw)/2:y=(oh-ih)/2:color=#00000000,scale=1920:1080,hqdn3d" "%OUTPUT%"
