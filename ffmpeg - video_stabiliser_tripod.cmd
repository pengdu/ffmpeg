...
%FFMPEG% -y -i "%~1" -preset ultrafast -an -vf "setpts=PTS-STARTPTS,vidstabdetect=tripod=1:result=%~sn1_stabilizer_data.trf"   -f null  nul
%FFMPEG% -y -i "%~1" -preset ultrafast     -vf "setpts=PTS-STARTPTS,vidstabtransform=tripod=1:input=%~sn1_stabilizer_data.trf" "%~dpn1__stabilized%~x1"
