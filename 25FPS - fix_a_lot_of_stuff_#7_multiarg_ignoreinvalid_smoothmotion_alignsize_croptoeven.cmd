...
set "FILE_INPUT=%~s1"
set "FILE_TEMP=%~d1%~p1%~n1_temp.mkv"
set "FILE_OUTPUT=%~d1%~p1%~n1_fixed.mkv"
set "FILE_STBLE=%~n1_transforms.trf"

set ARGS=

::program switches
set ARGS=%ARGS%  -y  -hide_banner   -threads "16"  -strict "experimental"
...
set ARGS=%ARGS% -vf "setpts=PTS-STARTPTS,yadif=0:-1:0,dejudder=cycle=20,mpdecimate,format=pix_fmts=yuv420p,crop=trunc(in_w/2)*2:trunc(in_h/2)*2,fps=fps=25,setpts=N/FRAME_RATE/TB-STARTPTS,format=pix_fmts=yuv420p"
...
::              align, deinterlace, fix chroma, adjust frames to 16:9 by padding, accurate frame-rate and set video-step-speed according to the frame-rate, faster video.
::set ARGS=%ARGS% -vf "setpts=PTS-STARTPTS,yadif=0:-1:0,dejudder=cycle=20,mpdecimate,format=pix_fmts=yuv420p,pad=width=ih*16/9:height=ih:x=(ow-iw)/2:y=(oh-ih)/2:color=#00000000,fps=fps=25,setpts=N/FRAME_RATE/TB-STARTPTS,format=pix_fmts=yuv420p"

::adjust length to total length of shorter video.
set ARGS=%ARGS% -shortest

::audio processing
set ARGS=%ARGS% -af "asetpts=PTS-STARTPTS,aresample=async=1:min_hard_comp=0.100000"

echo.
@echo on
call ffmpeg.exe %ARGS% "%FILE_TEMP%"
@echo off
echo.
echo.

echo Calculating stability-table (%FILE_STBLE%)...
@echo on
call ffmpeg.exe -y -i "%FILE_TEMP%" -an -sn -vf "setpts=PTS-STARTPTS,vidstabdetect=result=%FILE_STBLE%"   -f null  nul
@echo off
echo Done.
echo.
...
echo Stabling Video...
@echo on
call ffmpeg.exe -y -i "%FILE_TEMP%" -vf "setpts=PTS-STARTPTS,vidstabtransform=input=%FILE_STBLE%:smoothing=20:interpol=bicubic:zoom=5,unsharp=5:5:0.8:3:3:0.4"   "%FILE_OUTPUT%"
@echo off
echo Done.
echo.
echo.
